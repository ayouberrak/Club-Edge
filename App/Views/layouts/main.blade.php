    ...
    <header class="sticky top-0 z-50 py-8 px-6 transition-all duration-500" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)">
        <div class="container mx-auto">
            <div :class="scrolled ? 'py-4 px-8 rounded-[3rem] shadow-2xl bg-slate-950/80 border-white/5' : 'py-6 px-10 rounded-[3.5rem] bg-transparent border-transparent'" class="glass flex items-center justify-between transition-all duration-500 backdrop-blur-xl">
                <a href="{{ $base_url }}/" class="flex items-center space-x-4 group">
                    <img src="{{ $base_url }}/assets/img/logo.png" alt="Edge Logo" class="w-14 h-14 rounded-[1.3rem] shadow-[0_10px_30px_rgba(79,70,229,0.4)] transform group-hover:rotate-[12deg] transition-all duration-700 object-cover">
                ...
                <nav class="hidden lg:flex items-center space-x-14 text-[11px] font-black uppercase tracking-[0.4em] text-slate-500">
                    <a href="{{ $base_url }}/" class="hover:text-white transition-all relative group py-2">
                        <span>Directory</span>
                        <span class="absolute bottom-0 left-0 w-0 h-[2px] bg-indigo-500 group-hover:w-full transition-all duration-500"></span>
                    </a>
                    <a href="{{ $base_url }}/#clubs" class="hover:text-white transition-all relative group py-2">
                        <span>Universe</span>
                        <span class="absolute bottom-0 left-0 w-0 h-[2px] bg-indigo-500 group-hover:w-full transition-all duration-500"></span>
                    </a>
                    <a href="{{ $base_url }}/#clubs" class="hover:text-white transition-all relative group py-2">
                ...
                <div class="flex items-center space-x-8">
                    @if(isset($_SESSION['user_id']))
                        <div class="flex items-center space-x-6">
                            ...
                            @endphp
                            <a href="{{ $dashboardUrl }}" class="flex items-center space-x-3 group bg-indigo-500/5 hover:bg-indigo-500/10 px-4 py-2 rounded-2xl border border-indigo-500/10 transition-all">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-[10px] font-black text-white uppercase tracking-wider">{{ $_SESSION['user_nom'] ?? 'Account' }}</span>
                                    <span class="text-[8px] text-indigo-400 font-bold uppercase tracking-widest">Dashboard</span>
                                </div>
                            </a>

                            <a href="{{ $base_url }}/logout" class="text-slate-500 hover:text-red-400 transition-colors p-2" title="Sign Out">
