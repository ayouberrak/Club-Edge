...
<div class="py-10 flex flex-col lg:flex-row gap-8" x-data="{ 
    ...
    <main class="flex-grow space-y-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="glass p-6 rounded-3xl border border-slate-800 bg-gradient-to-br from-blue-500/5 to-transparent">
                ...
                <div class="text-3xl font-bold flex items-center">
                    <?php echo e($stats['total_clubs'] ?? 0); ?>
            ...
            <div class="glass p-6 rounded-3xl border border-slate-800 bg-gradient-to-br from-indigo-500/5 to-transparent">
                ...
                <div class="text-3xl font-bold"><?php echo e($stats['total_students'] ?? 0); ?></div>
            </div>
            <div class="glass p-6 rounded-3xl border border-slate-800 bg-gradient-to-br from-yellow-500/5 to-transparent">
                ...
                <div class="text-3xl font-bold text-yellow-500"><?php echo e($stats['pending_reviews'] ?? 0); ?></div>
            </div>
            <div class="glass p-6 rounded-3xl border border-slate-800 bg-gradient-to-br from-green-500/5 to-transparent">
                ...
                <div class="text-3xl font-bold text-green-500"><?php echo e($stats['active_events'] ?? 0); ?></div>
            </div>
        </div>
        ...
        <div x-show="activeSection === 'clubs'" class="space-y-6 animate-fadeIn">
            <div class="flex justify-between items-end">
                <h2 class="text-3xl font-bold">Platform Clubs <span class="text-slate-500 text-lg">(<?php echo e(count($clubs ?? [])); ?>)</span></h2>
                <div class="text-xs text-slate-500 italic">Target: 4-6 clubs per establishment</div>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <?php $__currentLoopData = $clubs ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $club): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="glass p-6 rounded-3xl border border-slate-800 relative overflow-hidden group hover:border-blue-500/40 transition-all cursor-pointer" onclick="window.location.href='<?php echo e($base_url); ?>/dashboard/admin/club/<?php echo e($club['id_club']); ?>'">
                        ...
                        <div class="flex items-start gap-4">
                            <div class="relative shrink-0">
                                <div class="w-16 h-16 rounded-2xl overflow-hidden border-2 border-slate-700 shadow-lg group-hover:border-blue-500/50 transition-colors">
                                    <img src="<?php echo e($base_url); ?>/public/assets/img/<?php echo e($club['image_url']); ?>" alt="<?php echo e($club['nom']); ?>" class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500">
                                </div>
                                <div class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-slate-900 <?php echo e(($club['club_members'] >= $club['max_membres'] && $club['max_membres'] > 0) ? 'bg-red-500' : 'bg-green-500'); ?>"></div>
                            </div>
                            <div>
                                <h3 class="text-xl font-bold leading-tight group-hover:text-blue-400 transition-colors"><?php echo e($club['nom']); ?></h3>
                                <p class="text-xs text-slate-500 mt-1 uppercase tracking-tighter">President: <span class="<?php echo e($club['id_president'] ? 'text-slate-300' : 'text-yellow-500'); ?> font-bold"><?php echo e($club['president'] ?? 'Not Assigned'); ?></span></p>
                            </div>
                        </div>
                        <div class="flex gap-2" @click.stop>
                            <button type="button" class="formunlleModifier w-9 h-9 flex items-center justify-center rounded-xl bg-slate-800 border border-slate-700 hover:bg-blue-600 hover:text-white transition-all" data-clubid="<?php echo e($club['id_club']); ?>"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>
                            <a href="<?php echo e($base_url); ?>/dashboard/admin/club/delete/<?php echo e($club['id_club']); ?>" class="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-800 border border-slate-700 hover:bg-red-600 hover:text-white transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></a>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 relative z-10">
                        <div class="space-y-2">
                            <div class="flex justify-between text-xs font-bold uppercase tracking-widest text-slate-500">
                                ...
                                <span class="<?php echo e(($club['club_members'] >= $club['max_membres'] && $club['max_membres'] > 0) ? 'text-red-400' : 'text-blue-400'); ?>"><?php echo e($club['club_members']); ?> / <?php echo e($club['max_membres']); ?></span>
                            </div>
                            <div class="w-full bg-slate-800 h-2 rounded-full overflow-hidden flex">
                                <div class="h-full <?php echo e(($club['club_members'] >= $club['max_membres'] && $club['max_membres'] > 0) ? 'bg-red-500' : 'bg-blue-500'); ?>" style="width: <?php echo e(($club['max_membres'] > 0) ? ($club['club_members'] / $club['max_membres'] * 100) : 0); ?>%"></div>
                            </div>
                        </div>
                        <div class="flex justify-between items-center mt-4 border-t border-slate-800 pt-3">
                            <span class="text-[10px] text-slate-500">Created: <?php echo e($club['created_at'] ?? '2024'); ?></span>
                        ...
                        <tr class="hover:bg-slate-800/30 transition-colors group">
                            <td class="py-6">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <div class="font-bold text-white group-hover:text-blue-400 transition-colors"><?php echo e($student['name']); ?></div>
                                        <div class="text-[10px] text-slate-500"><?php echo e($student['email']); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="py-6 font-mono text-xs text-slate-400">#<?php echo e($student['id']); ?></td>
                            <td class="py-6">
                                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase <?php echo e($student['club'] === 'None' ? 'bg-slate-800 text-slate-500' : 'bg-blue-600/10 text-blue-400 border border-blue-500/20'); ?>">
                                    <?php echo e($student['club']); ?>
                            ...
                            <td class="py-6">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <a href="<?php echo e($base_url); ?>/dashboard/admin/student/delete/<?php echo e($student['id']); ?>" onclick="return confirm('Are you sure you want to ban/delete this student?');" class="text-red-900 font-bold text-xs uppercase hover:text-red-500 transition-colors">Ban</a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div x-show="activeSection === 'logs'" class="glass p-8 rounded-3xl border border-slate-800 animate-fadeIn">
            ...
            <div class="space-y-3 font-mono text-xs text-slate-400">
                <div class="p-3 bg-slate-900/80 rounded-xl border border-slate-800 flex items-center">
                    ...
                    <span class="text-slate-500 mr-4"><?php echo e(date('H:i:s')); ?></span>
    ...
    <div x-show="showClubModal" class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-md" x-cloak x-transition>
        <div class="glass w-full max-w-lg p-10 rounded-[2.5rem] border border-blue-500/30 shadow-[0_0_50px_rgba(37,99,235,0.1)]" @click.away="showClubModal = false">
            ...
            <form class="space-y-4" method="post" action="<?php echo e($base_url); ?>/dashboard/admin/club/create" enctype="multipart/form-data">
                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="id_president" class="w-full bg-slate-900/50 border-2 border-slate-800 rounded-2xl px-4 py-3 text-white font-bold focus:border-blue-500 focus:outline-none transition-all appearance-none cursor-pointer text-sm">
                            ...
                            <?php $__currentLoopData = $potentialPresidents ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($user['id_user']); ?>" class="bg-slate-900"><?php echo e($user['nom']); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                </div>
...
<div id="modifyClubModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-md">
    <div class="glass w-full max-w-lg p-10 rounded-[2.5rem] border border-blue-500/30 shadow-[0_0_50px_rgba(37,99,235,0.1)]">
        ...
        <form id="modifyClubForm" class="space-y-4" enctype="multipart/form-data" method="post" action="<?php echo e($base_url); ?>/dashboard/admin/club/update">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        modificationBtns.forEach(ele => {
            ele.addEventListener('click', (e) => {
                ...
                fetch(`<?php echo e($base_url); ?>/dashboard/admin/club/edit/${clubid}`)
                    .then(rep => {
